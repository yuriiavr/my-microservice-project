...
Конфігурація читається зі змінних оточення (передаються через docker-compose).
"""
import os
...
# PostgreSQL — параметри підключення зі змінних оточення.
DATABASES = {
    "default": {
...
# Запити приходять через Nginx-проксі — довіряємо локальним origin для CSRF.
